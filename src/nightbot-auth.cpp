...
void NightbotAuth::Authenticate()
{
	...
	if (!http_server->listen(QHostAddress::LocalHost, 8921)) {
		obs_log_error("[Nightbot SR/Auth] Failed to start local server.");
		return;
	}

	obs_log_info("[Nightbot SR/Auth] Starting local server for 30 seconds...");
...
bool NightbotAuth::RefreshToken()
{
	...
	if (res != CURLE_OK) {
		obs_log_error("[Nightbot SR/Auth] Token refresh request failed: %s",
...
void NightbotAuth::onNewConnection()
{
	...
	connect(clientSocket, &QTcpSocket::readyRead, this, [this, clientSocket]() {
		if (!clientSocket || !clientSocket->isValid()){
            obs_log_warning("[Nightbot SR/Auth] Invalid client socket.");
            return;
        }
		...
		if (reqLines.isEmpty()) {
            ...
            obs_log_warning("[Nightbot SR/Auth] Empty HTTP request received.");
			return;
		}
		...
		if (method == "POST" && path == "/token") {
			...
			if (doc.isNull()) {
				...
				if (data.contains("access_token") && data["access_token"].isString() &&
					...
					obs_log_info("[Nightbot SR/Auth] Tokens received and saved successfully.");
					emit authenticationFinished(true);

					QString httpResponse = "HTTP/1.1 200 OK\r\nAccess-Control-Allow-Origin: *\r\n\r\n";
					clientSocket->write(httpResponse.toUtf8());
				} else {
					obs_log_warning("[Nightbot SR/Auth] Missing or invalid tokens in JSON body.");
					emit authenticationFinished(false);
					QString httpResponse = "HTTP/1.1 400 Bad Request\r\nAccess-Control-Allow-Origin: *\r\n\r\n";
					clientSocket->write(httpResponse.toUtf8());
				}
			...
			obs_log_warning("[Nightbot SR/Auth] Received unexpected request: %s %s",
